...
#' @importFrom stringr str_remove str_extract str_sub str_subset
...
#' @examples
#' \dontrun{
#' master_list <- list(project_details = list(script_log = list(timestamps = list(start_time = Sys.time()), runtimes = list(), messages = list())))
...
update_script_log <- function(master_list, section_name, previous_section_name, next_section_name) {
  # Validate parameters
  if (!section_name %in% names(master_list$project_details$script_log$timestamps)) {
    stop("Invalid section_name")
  }
  if (!previous_section_name %in% names(master_list$project_details$script_log$timestamps)) {
    stop("Invalid previous_section_name")
  }
  if (!next_section_name %in% names(master_list$project_details$script_log$timestamps)) {
    stop("Invalid next_section_name")
  }
...
#' @examples
#' \dontrun{
#' move_folder("path/to/source", "path/to/destination", 60)
#' }
...
#' }
skyline_setup_project <- function(project_directory, plateID) {
  # Validate parameters
...
mzR_mrm_findR <- function(FUNC_mzR, FUNC_mrm_guide, FUNC_OPTION_qc_type) {
      ...
      if (length(grep(idx_mzML, names(FUNC_mzR[[idx_plate]]))) == 1) {
        ...
        for (idx_mrm in 3:nrow(FUNC_mzR[[idx_plate]][[idx_mzML]]$mzR_header)) {
          ...
          if (nrow(FUNC_mzR[[idx_plate]][[idx_mzML]]$mzR_chromatogram[[idx_mrm]]) > 0) {
            ...
            baseline_value <- median(FUNC_mzR[[idx_plate]][[idx_mzML]]$mzR_chromatogram[[idx_mrm]][,2])
            if (baseline_value < 1) {
              baseline_value <- mean(FUNC_mzR[[idx_plate]][[idx_mzML]]$mzR_chromatogram[[idx_mrm]][,2])
            }
            ...
            peak_end_idx <- baseline_idx[which(baseline_idx > peak_apex_idx)]
            ...
            mzml_rt_apex <- round(FUNC_mzR[[idx_plate]][[idx_mzML]]$mzR_chromatogram[[idx_mrm]]$time[peak_apex_idx], 2)
            mzml_rt_start <- round(FUNC_mzR[[idx_plate]][[idx_mzML]]$mzR_chromatogram[[idx_mrm]]$time[peak_start_idx], 2)
            mzml_rt_end <- round(FUNC_mzR[[idx_plate]][[idx_mzML]]$mzR_chromatogram[[idx_mrm]]$time[peak_end_idx], 2)

            lipid_idx <- which(FUNC_mrm_guide$precursor_mz == precursor_mz & FUNC_mrm_guide$product_mz == product_mz)
            if (length(lipid_idx) != 1) {
              lipid_idx <- which(FUNC_mrm_guide$precursor_mz > (precursor_mz - 0.25) & FUNC_mrm_guide$precursor_mz < (precursor_mz + 0.25) & FUNC_mrm_guide$product_mz > (product_mz - 0.25) & FUNC_mrm_guide$product_mz < (product_mz + 0.25) & FUNC_mrm_guide$explicit_retention_time > (min(FUNC_mzR[[idx_plate]][[idx_mzML]]$mzR_chromatogram[[idx_mrm]]$time) - 0.1) & FUNC_mrm_guide$explicit_retention_time < (max(FUNC_mzR[[idx_plate]][[idx_mzML]]$mzR_chromatogram[[idx_mrm]]$time) + 0.1))
            }

            if (length(lipid_idx) > 1) {
              if (idx_mzML == mzML_filelist_qc) {
              ...
              if (idx_mzML == mzML_filelist_qc) {
                print(paste("no match for precursor =", precursor_mz, "; product =", product_mz, "; retention time =", mzml_rt_apex, "; transitions"))
              }
            ...
            FUNC_tibble[[idx_mzML]] <- bind_rows(FUNC_tibble[[idx_mzML]], tibble(
              mzml = idx_mzML,
  ...
  for (idx_lipid in unique(FUNC_tibble$lipid)) {
    FUNC_output$mrm_guide_updated <- bind_rows(FUNC_output$mrm_guide_updated, tibble(
      Molecule_List_Name = unique(FUNC_tibble %>% filter(lipid == idx_lipid) %>% pull(lipid_class)),
      Precursor_Name = idx_lipid,
      Precursor_Mz = FUNC_mrm_guide %>% filter(precursor_name == idx_lipid) %>% pull(precursor_mz),
      Precursor_Charge = FUNC_mrm_guide %>% filter(precursor_name == idx_lipid) %>% pull(precursor_charge),
      Product_Mz = FUNC_mrm_guide %>% filter(precursor_name == idx_lipid) %>% pull(product_mz),
      Product_Charge = FUNC_mrm_guide %>% filter(precursor_name == idx_lipid) %>% pull(product_charge),
      Explicit_Retention_Time = median(FUNC_tibble %>% filter(lipid == idx_lipid) %>% pull(peak_apex)),
      Explicit_Retention_Time_Window = FUNC_mrm_guide %>% filter(precursor_name == idx_lipid) %>% pull(explicit_retention_time_window),
      Note = FUNC_mrm_guide %>% filter(precursor_name == idx_lipid) %>% pull(note)
    ))

    FUNC_output$peak_boundary_update <- bind_rows(FUNC_output$peak_boundary_update, tibble(
      FileName = mzML_filelist,
      FullPeptideName = rep(idx_lipid, length(mzML_filelist)),
      MinStartTime = rep(summary(FUNC_tibble %>% filter(lipid == idx_lipid) %>% pull(peak_start))["1st Qu."], length(mzML_filelist)),
      MaxEndTime = rep(summary(FUNC_tibble %>% filter(lipid == idx_lipid) %>% pull(peak_end))["3rd Qu."], length(mzML_filelist))
    ))
  }

  FUNC_output$mrm_guide_updated <- FUNC_output$mrm_guide_updated %>% arrange(Precursor_Name)
...
peak_picking <- function(plateID, master_list) {
  ...
  for (plate_idx in master_list$project_details$plateID) {
    ...
    if (any(grepl("LTR", master_list$project_details$mzml_sample_list[[plate_idx]]))) {
      ...
      print("mzml files contain neither LTR nor PQC. SkylineR is unable to process your data.")
    }
  ...
  # Update script log
  master_list <- update_script_log(master_list, "peak_picking", "mzml_file_processing", "data_analysis")
...
qcCheckR_setup_project <- function(project_directory) {
  ...
  master_list$templates <- list()
  master_list$templates$v1$SIL_guide <- read_csv(file = paste0(master_list$project_details$github_master_dir, "templates/LGW_lipid_mrm_template_v1.csv"), show_col_types = FALSE) %>%
    clean_names() %>%
    standard_convention()

  master_list$templates$v1$conc_guide <- read_csv(file = paste0(master_list$project_details$github_master_dir, "templates/LGW_SIL_batch_103.csv"), show_col_types = FALSE) %>%
    clean_names() %>%
    standard_convention()

  master_list$templates$v2$SIL_guide <- read_csv(file = paste0(master_list$project_details$github_master_dir, "templates/LGW_lipid_mrm_template_v2.csv"), show_col_types = FALSE) %>%
    clean_names() %>%
    standard_convention()

  master_list$templates$v2$conc_guide <- read_csv(file = paste0(master_list$project_details$github_master_dir, "templates/LGW_SIL_batch_Ultimate_2023-03-06.csv"), show_col_types = FALSE) %>%
    clean_names() %>%
    standard_convention()

  master_list$templates$v3$SIL_guide <- read_csv(file = paste0(master_list$project_details$github_master_dir, "templates/LGW_lipid_mrm_template_v3.csv"), show_col_types = FALSE) %>%
    clean_names() %>%
    standard_convention()

  master_list$templates$v3$conc_guide <- read_csv(file = paste0(master_list$project_details$github_master_dir, "templates/LGW_SIL_batch_Ultimate_2023-03-06.csv"), show_col_types = FALSE) %>%
    clean_names() %>%
    standard_convention()

  master_list$templates$v4$SIL_guide <- read_csv(file = paste0(master_list$project_details$github_master_dir, "templates/LGW_lipid_mrm_template_v4.csv"), show_col_types = FALSE) %>%
    clean_names() %>%
    standard_convention()

  master_list$templates$v4$conc_guide <- read_csv(file = paste0(master_list$project_details$github_master_dir, "templates/LGW_SIL_batch_Ultimate_2023-03-06.csv"), show_col_types = FALSE) %>%
    clean_names() %>%
    standard_convention()
...
qcCheckR_export_html_report <-  function(master_list){
  ...
  ### 2. render template
  rmarkdown::render(input ="C:/Users/Hszem/OneDrive - Murdoch University/Masters THESIS/Documents/GitHub/targeted_lipid_exploreR/v6_Automated/templates/lipid_exploreR_report_template_v5.R",
