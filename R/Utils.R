...
#'
#' #' Check and Prompt Suggested Packages
#' #'
#' #' This function reads the DESCRIPTION file of MetaboExploreR, checks for suggested packages,
#' #' and prompts the user to install any missing packages.
#' #'
#' #' @param description_path Character. MetaboExploreR DESCRIPTION file. Default is "DESCRIPTION".
#' #' @return Invisible TRUE if all suggested packages are installed, FALSE otherwise prompts
#' #'  user to install missing packages
#' #' @examples
#' #' \dontrun{
#' #' check_and_prompt_suggested_packages()
#' #' }
#' check_and_prompt_suggested_packages <- function(description_path = "DESCRIPTION") {
#'   # Read DESCRIPTION file
#'   lines <- readLines(description_path)
#'
#'   # Find the start of the Suggests field
#'   start_idx <- grep("^Suggests:", lines)
#'   if (length(start_idx) == 0) {
#'     message("No 'Suggests:' field found in DESCRIPTION.")
#'     return(invisible(TRUE))
#'   }
#'
#'   # Collect all lines that belong to the Suggests field
#'   suggests_lines <- lines[start_idx]
#'   i <- start_idx + 1
#'   while (i <= length(lines) && grepl("^\\s", lines[i])) {
#'     suggests_lines <- c(suggests_lines, lines[i])
#'     i <- i + 1
#'   }
#'
#'   # Collapse and clean
#'   suggests_raw <- paste(suggests_lines, collapse = " ")
#'   suggests_raw <- sub("^Suggests:\\s*", "", suggests_raw)
#'   pkg_names <- unlist(strsplit(gsub("\\s*\\(.*?\\)", "", suggests_raw), ",\\s*"))
#'   print(pkg_names)
#'
#'   # Check which are missing
#'   missing <- pkg_names[!sapply(pkg_names, requireNamespace, quietly = TRUE)]
#'
#'   if (length(missing) == 0) {
...
#'   }
#'
#'   message("ERROR: Missing required packages:\n  - ", paste(missing, collapse = "\n  - "))
#'   response <- readline("Would you like to install them now? [y/n]: ")
#'   if (tolower(response) != "y") {
#'     message("Skipping installation. You are unable to run MetaboExploreR without these packages")
...
#'   }
#'
#'   # Known Bioconductor packages (expand as needed)
#'   bioc_pkgs <- c("mzR", "ropls", "statTarget")
#'
